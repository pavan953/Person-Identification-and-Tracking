...
camera_ips = [
    "http://192.168.232.28:8080/video",
]
    ...
    for i, frame in enumerate(frames):
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if usn == usn_to_track:
                ...
                if not person_found:
                    person_found = True
                    # Send notification to the Flask server
                    requests.post('http://localhost:5000/api/person_found', json={'camera': f'Camera {i}', 'name': name})
            else:
...
#     # "http://192.168.101.106:8080/video",
#     "http://192.168.101.85:8080/video",
#     # "http://192.168.101.126:8080/video",
#     # Add more camera IPs as needed
# ]
...
#                 # Send notification to the Flask server
#                 try:
#                     requests.post('http://localhost:5000/api/person_found', json={'camera': f'Camera {index}', 'name': name})
...
#             thread = threading.Thread(target=process_frame, args=(i, frame))
#             thread.daemon = True
#             thread.start()
